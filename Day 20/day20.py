...
class Droid:
    ...
    def clone(self, direction):
        return Droid(self.path, self.point, direction, self.portals)
    ...
    def attempt_move(self, maze_map):
        ...
        def can_move_to_space(_next_char):
            return _next_char is not None and (_next_char == SPACE or (maze_map[self.point].isupper() and _next_char.isupper))
        ...
        if can_move:
            ...
            print(get_teleport_key(next_char, next_point, maze_map))
        else:
            droid_to_remove = self
...
def get_teleport_key(char, point, maze_map):
    ...
    if point2.x < point.x or point2.y < point.y:
        return char2 + char
    return char + char2
...
def part_one():
    ...
    start_point = portal_map.get("AA")[0][0]
